...
public class ReactiveKafkaConfiguratorProject extends ReactiveKafkaConfigurator {
	...
	@Bean ReactiveStreamDispatcher<Mail> mailDispatcher(){
		return new ReactiveStreamDispatcher<>(Mail.class, reactiveKafkaProperties, Streams.MAIL_CHANNEL_OUTPUT);
	}

	@Bean ReactiveStreamDispatcher<Message> messageDispatcher(){
		return new ReactiveStreamDispatcher<>(Message.class, reactiveKafkaProperties, Streams.MESSAGE_CHANNEL_OUTPUT);
	}

	@Bean ReactiveStreamDispatcher<Mms> mmsDispatcher(){
		return new ReactiveStreamDispatcher<>(Mms.class, reactiveKafkaProperties, Streams.MMS_CHANNEL_OUTPUT);
	}

	@Bean ReactiveStreamDispatcher<Sms> smsDispatcher(){
		return new ReactiveStreamDispatcher<>(Sms.class, reactiveKafkaProperties, Streams.SMS_CHANNEL_OUTPUT);
	}

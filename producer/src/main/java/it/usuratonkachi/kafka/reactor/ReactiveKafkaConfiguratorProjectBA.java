...
public class ReactiveKafkaConfiguratorProjectBA extends ReactiveKafkaConfigurator {
	...
	ReactiveStreamDispatcherBA<Mail> mailDispatcher(){
		return new ReactiveStreamDispatcherBA<>(reactiveKafkaProperties, Streams.MAIL_CHANNEL_OUTPUT);
	}

	@Bean
	ReactiveStreamDispatcherBA<Message> messageDispatcher(){
		return new ReactiveStreamDispatcherBA<>(reactiveKafkaProperties, Streams.MESSAGE_CHANNEL_OUTPUT);
	}

	@Bean
	ReactiveStreamDispatcherBA<Mms> mmsDispatcher(){
		return new ReactiveStreamDispatcherBA<>(reactiveKafkaProperties, Streams.MMS_CHANNEL_OUTPUT);
	}

	@Bean ReactiveStreamDispatcherBA<Sms> smsDispatcher(){
		return new ReactiveStreamDispatcherBA<>(reactiveKafkaProperties, Streams.SMS_CHANNEL_OUTPUT);
	}

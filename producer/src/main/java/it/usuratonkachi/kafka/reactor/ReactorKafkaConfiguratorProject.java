...
public class ReactorKafkaConfiguratorProject extends ReactorKafkaConfigurator {
	...
	@Bean ReactorStreamDispatcher<Mail> mailDispatcher(){
		return new ReactorStreamDispatcher<>(Mail.class, reactorKafkaProperties, Streams.MAIL_CHANNEL_OUTPUT);
	}

	@Bean ReactorStreamDispatcher<Message> messageDispatcher(){
		return new ReactorStreamDispatcher<>(Message.class, reactorKafkaProperties, Streams.MESSAGE_CHANNEL_OUTPUT);
	}

	@Bean ReactorStreamDispatcher<Mms> mmsDispatcher(){
		return new ReactorStreamDispatcher<>(Mms.class, reactorKafkaProperties, Streams.MMS_CHANNEL_OUTPUT);
	}

	@Bean ReactorStreamDispatcher<Sms> smsDispatcher(){
		return new ReactorStreamDispatcher<>(Sms.class, reactorKafkaProperties, Streams.SMS_CHANNEL_OUTPUT);
	}

...
public class ReactiveStreamDispatcherBA<T> {
	...
	private ProducerRecord<String, byte[]> messageToProducerRecord(Message<T> message){
		if (message.getPayload() instanceof Serializable) {
			return new ProducerRecord<>(reactiveKafkaConfiguration.getTopicName(), null, null, null, SerializationUtils.serialize((Serializable)message.getPayload()), headersToProducerRecordHeaders(message));
		}
		throw new RuntimeException("Cannot serialize object of class " + this.clazz);
	}
